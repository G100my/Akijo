...
function onEdit(e) {
  ...
  // 檢查是否更改班別表
  const isEditingTimeRange =
    editedCol >= 33 && editedCol <= 36 && editedRow >= rs && editedRow <= re

  console.log(editedRow, editedCol, isEditingTimeRange)
  // 檢測編輯是否發生在星期行或職位狀態列
  if (
    editedRow === weekDayRow ||
    editedCol === positionStatusCol ||
    isEditingTimeRange
  ) {
...
function applyValidationsForAll(sheet) {
  ...
  // 獲取星期和職位狀態的資料
  const weekDays = sheet
    .getRange(
      `${dataRangeStartCol}${weekDayRow}:${dataRangeEndCol}${weekDayRow}`,
    )
    .getValues()[0]
  const positionStatuses = sheet
    .getRange(
      `${columnToLetter(positionStatusCol)}${positionStatusRowStart}:${columnToLetter(positionStatusCol)}${positionStatusRowEnd}`,
    )
    .getValues()
    .flat()
